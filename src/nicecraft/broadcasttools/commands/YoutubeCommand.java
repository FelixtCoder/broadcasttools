...
public class YoutubeCommand implements CommandExecutor {
	...
	public YoutubeCommand (BroadcastTools plugin) {
		this.plugin = plugin;
	}
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		return false;
			...
			if(args.length > 0) {
				if(player.isPermissionSet("bct.youtube") || player.isPermissionSet("bct.admin") || player.isOp()) {
					String message = config.getString("Config.aviso-youtube").replaceAll("%link%", args[0]);
